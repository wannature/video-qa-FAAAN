...
class GRA(object):
    ...
    def build_inference(self):
        ...
        with tf.variable_scope('init'):
            ...
            v_state = a_cell.zero_state(batch_size, tf.float32)
        ...
        def _one_step(time, q_state, a_state,v_state, appear_weight, motion_weight, channel_weight, fused):
            ...
            with tf.variable_scope('attend_1'):
            ...
            with tf.variable_scope('lstm_s'):
                s_output, s_state = a_cell(word, a_state)
            ...
            with tf.name_scope('attend_2'):
                ...
                weight_2, att_2 = self.attend(
                    s_output, current_video_att)
                sentence=weight_2*s_output

            with tf.variable_scope('lstm_v'):
                v_output, v_state = a_cell(current_video_att, v_state)
                with tf.name_scope('attend_3'):
                    ...
                    fuse_v = tf.expand_dims(channel_weight[:, 0], 1)
                    fuse_s = tf.expand_dims(channel_weight[:, 1], 1)
                    fused = fuse_v * sentence + fuse_s * v_output
